...
void search_nearest_point_backward_cuda(
    at::Tensor points, at::Tensor verts, at::Tensor faces,
    at::Tensor near_faces, at::Tensor grad);

// #if (TORCH_VERSION_MINOR<=1) && (TORCH_VERSION_MINOR<5)
...
void search_nearest_point_backward(
    ...
    CHECK_CUDA(near_faces);
    CHECK_CUDA(grad);
    search_nearest_point_backward_cuda(points, verts, faces, near_faces, grad);
}
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("search_nearest_point_backward", &search_nearest_point_backward, "NEAREST_BACKWARD (CUDA)");
}
