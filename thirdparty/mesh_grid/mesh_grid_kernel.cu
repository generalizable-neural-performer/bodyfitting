...
template<typename scalar_t, typename index, unsigned char dim>
__global__ void search_nearest_point_backward_kernel(
            const scalar_t *verts,  const index *tri, const scalar_t *points, const index points_num,
            const index *near_idx_, scalar_t *grad_) {
        const int id = blockIdx.x * blockDim.x + threadIdx.x;
        const scalar_t *p = points + 3 * id;
        index i = near_idx_[id];
        scalar_t coeff[dim], patch[dim*dim], inv[(dim+1)*(dim+1)];
	for(unsigned char d = 0; d < dim; ++d) {
		for(unsigned char d_= 0; d_< dim; ++d_)
			inv[d_+d*(dim+1)] = patch[d_+ d*dim] = verts[d_+dim*tri[d+dim*i]] - p[d_];
                inv[dim+d*(dim+1)] = 1;
                inv[d+dim*(dim+1)] = 1;
	}
        inv[(dim+1)*(dim+1)-1] = 0;
     	scalar_t dist2 = search_nearest_proj<scalar_t>(patch, coeff);
//      inverse4(inv, inv);
// TODO: calculate inverse matrix
        for(unsigned char i = 0; i < dim; ++i)
            for(unsigned char j = 0; j < dim; ++j)
                for(unsigned char k = 0; k < dim; ++k) {
                    grad_[k+dim*(j+dim*i)] = 0;
                    for(unsigned char l = 0; l < dim; ++l)
						grad_[k+dim*(j+dim*i)] -= (
                            inv[i+l*(dim+1)]*inv[k+dim*(dim+1)] +
                            inv[i+k*(dim+1)]*inv[l+dim*(dim+1)])*
                            patch[j+l*dim];
                }
}
...
void search_nearest_point_backward_cuda(
    at::Tensor points, at::Tensor verts, at::Tensor faces, at::Tensor near_faces, at::Tensor grad) {
    if(points.sizes().size() != 2) points = points.reshape({-1,3});
    int32_t points_num = points.size(0);
    ...
    // make output
    grad.resize_({points_num, 3, 3, 3});
    grad.zero_();
    AT_DISPATCH_FLOATING_TYPES(verts.type(), "search_nearest_point_cuda", ([&] {
        search_nearest_point_backward_kernel<scalar_t, int32_t, 3><<<blocks, threads>>>(
            verts.data<scalar_t>(),
            faces.data<int32_t>(),
            points.data<scalar_t>(),
            points_num,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
            printf("Error in search_nearest_point_backward_cuda: %s\n", cudaGetErrorString(err));
}
template<typename scalar_t>
